...
def popsum2d(pops, Nerls=Nerls):
    ...
    popSum[1] = np.sum(pops[1:(Nerls[0]+1)], axis = 0)            # E__
    popSum[2] = np.sum(pops[  (Nerls[0] +1):(2*Nerls[0]+1)], axis = 0)       # Et_
    popSum[3] = np.sum(pops[                (2*Nerls[0]+1):(3*Nerls[0]+1)], axis = 0)     # Es_
    popSum[4] = np.sum(pops[                               (3*Nerls[0]+1):(3*Nerls[0]+Nerls[1]+1)], axis = 0)     # P__
    popSum[5] = np.sum(pops[                                              (3*Nerls[0]+Nerls[1]+1):(3*Nerls[0]+2*Nerls[1]+1)], axis = 0)     # Pt_
    popSum[6] = np.sum(pops[                                                                      (3*Nerls[0]+2*Nerls[1]+1):(3*Nerls[0]+3*Nerls[1]+1)], axis = 0)     # Ps_
    popSum[7] = np.sum(pops[                                                                                                (3*Nerls[0]+3*Nerls[1]+1):(3*Nerls[0]+3*Nerls[1]+Nerls[2]+1)], axis = 0)     # I_p
    popSum[8] = np.sum(pops[                                                                                                                          (3*Nerls[0]+3*Nerls[1]+Nerls[2]+1):(3*Nerls[0]+3*Nerls[1]+Nerls[2]+Nerls[3]+1)], axis = 0)     # I_h
    popSum[9] = np.sum(pops[                                                                                                                                                             (3*Nerls[0]+3*Nerls[1]+Nerls[2]+Nerls[3]+1):(3*Nerls[0]+3*Nerls[1]+Nerls[2]+Nerls[3]+Nerls[4]+1)], axis = 0)     # I_i
    popSum[10] = np.sum(pops[                                                                                                                                                                                                        (3*Nerls[0]+3*Nerls[1]+Nerls[2]+Nerls[3]+Nerls[4]+1):(3*Nerls[0]+3*Nerls[1]+Nerls[2]+2*Nerls[3]+Nerls[4]+1)], axis = 0)    # Ish
    popSum[11] = np.sum(pops[                                                                                                                                                                                                                                                             (3*Nerls[0]+3*Nerls[1]+Nerls[2]+2*Nerls[3]+Nerls[4]+1):(3*Nerls[0]+3*Nerls[1]+2*Nerls[2]+2*Nerls[3]+Nerls[4]+1)], axis = 0)   # Isp
...
def plotEbola(names, savename, path=path, col=colsA, lab=lab, type='all', Nerls=Nerls):
    index = indexFunction(Nerls)
